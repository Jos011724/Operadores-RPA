...
function Dashboard({ token }) {
  ...
  const [formParBaterias, setFormParBaterias] = useState({
    id_bat1: "",
    ciclos1: "",
    id_bat2: "",
    ciclos2: "",
    ...
    try {
      await axios.post(
        "${API_URL}/api/vuelos",
        {
          ...
          equipo_id: dronEnTurno._id,
          base: baseEnTurno,
        },
        config,
      );
      setMensaje("✅ Vuelo guardado correctamente");
      cargarDatos();
      ...
      setMensaje(`❌ Error: ${e.response?.data?.error}`);
    }
  };
  ...
  const handleUpdatePar = async (e) => {
    ...
    try {
      await axios.put(
        `${API_URL}/api/baterias/ciclos/${id_bat1}`,
        { nuevos_ciclos: ciclos1, observacion },
        config,
      );
      if (id_bat2 && ciclos2)
        await axios.put(
          `${API_URL}/api/baterias/ciclos/${id_bat2}`,
          { nuevos_ciclos: ciclos2, observacion },
          config,
        );
      setMensaje("✅ Ciclos actualizados");
      setFormParBaterias({
        id_bat1: "",
        ciclos1: "",
        id_bat2: "",
  ...
  const handleCrearUsuario = async (e) => {
    ...
    try {
      ...
      setMensaje(`❌ Error: ${e.response?.data?.error || "Error desconocido"}`);
    }
  };

  const handleCrearDron = async (e) => {
    e.preventDefault();
    try {
      await axios.post("${API_URL}/api/drones", newDrone, config);
      setMensaje("Dron OK");
      cargarDatos();
    } catch (e) {
      setMensaje("Error Dron");
    }
  };
  const handleCrearBateria = async (e) => {
    e.preventDefault();
    if (!newBattery.equipo_id) return alert("Asigna equipo");
    try {
      await axios.post("${API_URL}/api/baterias", newBattery, config);
      setMensaje("Batería OK");
      cargarDatos();
    } catch (e) {
      setMensaje("Error Batería");
    }
  };
  const handleToggleUser = async (u) => {
    try {
      await axios.put(
        `${API_URL}/api/users/${u._id}`,
        { activo: !u.activo },
        config,
      );
      cargarDatos();
  ...
  const handleDeleteDrone = async (id) => {
    if (
      window.confirm("⚠️ ¿ELIMINAR EQUIPO?\nSe perderá el historial asociado.")
    ) {
      try {
        await axios.delete(`${API_URL}/api/drones/${id}`, config);
        cargarDatos();
      } catch (e) {
        alert("Error al eliminar");
      }
  ...
  const handleDeleteBattery = async (id) => {
    if (window.confirm("⚠️ ¿ELIMINAR BATERÍA?")) {
      try {
        await axios.delete(`${API_URL}/api/baterias/${id}`, config);
        cargarDatos();
      } catch (e) {
        alert("Error al eliminar");
      }
  ...
  const misBaterias = dronEnTurno
    ? baterias.filter((b) => b.equipo_id?._id === dronEnTurno._id)
    : [];
      ...
      <nav className="menu">
        {rol === "operador" && (
          <>
            {!dronEnTurno && (
              <button
                className={`menu-btn ${activeTab === "inicio_turno" ? "active" : ""}`}
                onClick={() => setActiveTab("inicio_turno")}
              >
                📍 Iniciar Turno
              </button>
            )}
            {dronEnTurno && (
              <button
                className={`menu-btn ${activeTab === "bitacora" ? "active" : ""}`}
            ...
            {dronEnTurno && (
              <button
                className={`menu-btn ${activeTab === "cierre" ? "active" : ""}`}
        ...
        {rol === "admin" && (
          <>
            <button
              className={`menu-btn ${activeTab === "admin_users" ? "active" : ""}`}
              onClick={() => setActiveTab("admin_users")}
            >
              👥 Usuarios
            </button>
            <button
              className={`menu-btn ${activeTab === "admin_drones" ? "active" : ""}`}
              onClick={() => setActiveTab("admin_drones")}
            >
              🚁 Equipos
            </button>
            <button
              className={`menu-btn ${activeTab === "admin_batteries" ? "active" : ""}`}
              onClick={() => setActiveTab("admin_batteries")}
            >
              🔋 Baterías
            </button>
          </>
          ...
          <div className="card">
            ...
            <div style={{ display: "grid", gap: "20px" }}>
              <div>
                ...
                <select
                  id="selBase"
                  onChange={(e) => setBaseEnTurno(e.target.value)}
                >
          ...
          <>
            ...
            >
              ...
              <div>
                <button
                  className="action-btn"
                  style={{ background: "var(--primary)", marginRight: "10px" }}
                  onClick={() => setMostrarModalBitacora(true)}
                >
                  Ver Historial
                </button>
                <button
            ...
            <div className="card">
              ...
              >
                <div style={{ gridColumn: "1/-1" }}>
                  ...
                  <input
                    type="date"
                    value={formVuelo.fecha}
                    onChange={(e) =>
                      setFormVuelo({ ...formVuelo, fecha: e.target.value })
                    }
                    required
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    type="time"
                    onChange={(e) =>
                      setFormVuelo({ ...formVuelo, hora_fin: e.target.value })
                    }
                    required
                  />
                </div>
                ...
                >
                  Duración: <strong>{duracionCalc} minutos</strong>
                </div>
                <div>
                  <label>Predio/Sector:</label>
                  <input
                    placeholder="Ej: Sector Norte"
                    onChange={(e) =>
                      setFormVuelo({ ...formVuelo, predio: e.target.value })
                    }
                    required
                  />
                </div>
            ...
            <form onSubmit={handleUpdatePar}>
                ...
                >
                  ...
                  {misBaterias.map((b) => (
                    <option key={b._id} value={b._id}>
                      {b.serial} (Actual: {b.ciclos})
                    </option>
                  ))}
                </select>
                <input
                  type="number"
                  placeholder="Ingresar Nuevos Ciclos"
                  onChange={(e) =>
                    setFormParBaterias({
                      ...formParBaterias,
                      ciclos1: e.target.value,
                    })
                  }
                  required
                />
              ...
              >
                <label style={{ color: "var(--primary)" }}>
                  2. Batería Secundaria (Opcional)
                </label>
                <select
                  onChange={(e) =>
                    setFormParBaterias({
                      ...formParBaterias,
                      id_bat2: e.target.value,
                    })
                  }
                >
                  <option value="">Ninguna / Solo una</option>
                  {misBaterias
                    .filter((b) => b._id !== formParBaterias.id_bat1)
                    .map((b) => (
                      <option key={b._id} value={b._id}>
                        {b.serial} (Actual: {b.ciclos})
                      </option>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormParBaterias({
                      ...formParBaterias,
                      ciclos2: e.target.value,
                    })
                  }
                  disabled={!formParBaterias.id_bat2}
                />
              </div>
              <label>Observaciones:</label>
              <textarea
            ...
            <div className="card">
              ...
              >
                <input
                  ...
                  value={newUser.run}
                  onChange={(e) =>
                    setNewUser({ ...newUser, run: e.target.value })
                  }
                  required
                />
                <input
        ...
        {activeTab === "admin_drones" && (
          <>
            <div className="card">
              ...
              <form
                onSubmit={handleCrearDron}
                style={{ gridTemplateColumns: "1fr 1fr" }}
              >
                <input
                  placeholder="Matrícula"
                  onChange={(e) =>
                    setNewDrone({ ...newDrone, matricula: e.target.value })
                  }
                  required
                />
                <input
                  placeholder="Modelo"
                  onChange={(e) =>
                    setNewDrone({ ...newDrone, modelo: e.target.value })
                  }
                  required
                />
                <button className="action-btn" style={{ gridColumn: "1/-1" }}>
                  Guardar Equipo
                </button>
              </form>
            </div>
                ...
                <tbody>
                  {drones.map((d) => (
                    <tr key={d._id}>
                      <td>{d.matricula}</td>
                      <td>{d.modelo}</td>
                      <td>{d.horas_vuelo.toFixed(1)} hrs</td>
                      <td>
                        <button
                          className="btn-icon btn-delete"
                          onClick={() => handleDeleteDrone(d._id)}
                        >
                          🗑️
                        </button>
                      </td>
        ...
        {activeTab === "admin_batteries" && (
          <>
            <div className="card">
              ...
              <form
                onSubmit={handleCrearBateria}
                style={{ gridTemplateColumns: "1fr 1fr" }}
              >
                <input
                  placeholder="Número de Serie"
                  onChange={(e) =>
                    setNewBattery({ ...newBattery, serial: e.target.value })
                  }
                  required
                />
                <select
                  onChange={(e) =>
                    setNewBattery({ ...newBattery, equipo_id: e.target.value })
                  }
                  required
                >
            ...
            <div className="card table-container">
              ...
              <table>
                ...
                <tbody>
                  {baterias.map((b) => (
                    <tr key={b._id}>
                      <td>{b.equipo_id?.matricula || "N/A"}</td>
                      <td style={{ color: "var(--warning)" }}>{b.serial}</td>
                      <td>{b.ciclos}</td>
                      <td>
                        <button
                          className="btn-icon btn-delete"
                          onClick={() => handleDeleteBattery(b._id)}
                        >
                          🗑️
                        </button>
                      </td>
          ...
          <div className="modal-content">
            ...
            >
              ...
              <button
                className="action-btn"
                style={{ background: "var(--danger)" }}
                onClick={() => setMostrarModalBitacora(false)}
              >
                Cerrar
              </button>
            </div>
            <table>
              ...
              <tbody>
                {vuelos
                  .filter((v) => v.operador_id?._id === datosUsuario.id)
                  .map((v) => (
                    <tr key={v._id}>
                      <td>{new Date(v.fecha).toLocaleDateString()}</td>
                      <td>{v.predio}</td>
                      <td>{v.duracion_min} min</td>
                    </tr>
                  ))}
              </tbody>
            </table>
