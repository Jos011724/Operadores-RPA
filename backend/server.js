...
mongoose.connect(MONGO_DB_URI)
  .then(async () => {
    ...
    console.log('👑 SISTEMA LISTO - Los datos de usuarios ahora son persistentes.');
  })
...
app.post('/api/auth/login', async (req, res) => {
  ...
  try {
    ...
    if (!user || !user.activo) return res.status(401).json({ error: 'Usuario no válido o inactivo' });
...
app.post('/api/users', verificarToken, verificarAdmin, async (req, res) => {
  try {
    ...
    if (!run || !password || !nombre || !apellido) return res.status(400).json({ error: 'Faltan datos obligatorios' });
    
    const existe = await User.findOne({ run });
    if (existe) return res.status(400).json({ error: 'El R.U.N. ya está registrado' });

    const hashedPassword = await bcrypt.hash(password, 10);
    const nuevoUsuario = await User.create({ run, nombre, apellido, password: hashedPassword, rol, activo: true });
    
    res.json(nuevoUsuario);
  } catch (e) { res.status(500).json({ error: 'Error al crear usuario: ' + e.message }); }
});
...
app.post('/api/drones', verificarToken, verificarAdmin, async (req, res) => {
  try { res.json(await Drone.create(req.body)); } catch(e) { res.status(400).json({error: 'Matrícula duplicada'}); }
});
app.delete('/api/drones/:id', verificarToken, verificarAdmin, async (req, res) => {
...
app.post('/api/baterias', verificarToken, verificarAdmin, async (req, res) => {
    try { res.json(await Battery.create(req.body)); } catch(e) { res.status(400).json({error: 'Serial duplicado'}); }
});
app.put('/api/baterias/ciclos/:id', verificarToken, async (req, res) => {
    const { nuevos_ciclos, observacion } = req.body;
    await Battery.findByIdAndUpdate(req.params.id, { ciclos: nuevos_ciclos, observacion });
    res.json({ message: 'Actualizado' });
});
app.delete('/api/baterias/:id', verificarToken, verificarAdmin, async (req, res) => {
...
app.get('/api/vuelos', verificarToken, async (req, res) => {
  const vuelos = await Flight.find().populate('operador_id', 'run nombre apellido').populate('equipo_id', 'matricula').sort({ fecha: -1 });
  res.json(vuelos);
});
app.post('/api/vuelos', verificarToken, async (req, res) => {
  try {
    const { equipo_id, base, hora_inicio, hora_fin, duracion_min, predio, coordenadas, observacion, fecha } = req.body;
    const vuelo = await Flight.create({ operador_id: req.usuario.id, equipo_id, base, hora_inicio, hora_fin, duracion_min, predio, coordenadas, observacion, fecha });
    await Drone.findByIdAndUpdate(equipo_id, { $inc: { horas_vuelo: duracion_min / 60 } });
    res.json(vuelo);
